...
import {brand, brandRgb, stateColor, ui, particle} from '../../styles/colors';
...
function drawRocket(
  ...
  ctx.shadowColor = brand.soft;
  ctx.shadowBlur = 10;

  ctx.beginPath();
  ctx.ellipse(0, 0, s * 0.42, s, 0, 0, Math.PI * 2);
  ctx.fillStyle = ui.rocketBody;
  ctx.fill();
...
export function CrashGame() {
  ...
  const spawnTrail = useCallback((x: number, y: number, angle: number) => {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      particlesRef.current.push({
        ...
        color:
          particle.trail[Math.floor(Math.random() * particle.trail.length)],
        life: 0,
  ...
  const spawnExplosion = useCallback((x: number, y: number) => {
    const colors = particle.explosion;
    for (let i = 0; i < 70; i++) {
  ...
  const getCoords = useCallback(
    (elapsedMs: number, mult: number, W: number, H: number) => {
  ...
  const draw = useCallback(
    (elapsedMs: number, crashed: boolean, cashedOut: boolean) => {
      ...
      const lineColor1 = crashed
        ? stateColor.crash
        : cashedOut
          ? stateColor.win
          : brand.vivid;
      const lineColor2 = crashed
        ? stateColor.crash
        : cashedOut
          ? stateColor.winLight
          : brand.soft;
      const fillAlpha = crashed ? 0.12 : cashedOut ? 0.12 : 0.18;
      const fillColor = crashed
        ? stateColor.crashRgb
        : cashedOut
          ? stateColor.winRgb
          : brandRgb.vivid;
      ...
      const glowColor2 = crashed
        ? `rgba(${stateColor.crashRgb},`
        : cashedOut
          ? `rgba(${stateColor.winRgb},`
          : `rgba(${brandRgb.soft},`;
      ...
      if (!crashed) {
        ...
        glow.addColorStop(0, `rgba(${stateColor.crashRgb},0.8)`);
        ...
        ctx.fillStyle = stateColor.crash;
        ctx.fill();
      }
      ...
      if (count <= 0) {
        ...
        const tick = () => {
          ...
          if (mult >= crashPointRef.current) {
            ...
            const canvas = canvasRef.current;
            if (canvas) {
              const {toX, toY, elapsedSec} = getCoords(
                elapsed,
                crashPointRef.current,
            ...
            setState('crashed');
            setHistory((prev) =>
              [{value: crashPointRef.current, won: false}, ...prev].slice(0, 8),
            );
            return;
          }
        ...
        <div className="max-w-2xl mx-auto">
              ...
              <AnimatePresence>
                {history.map((h, i) => (
                  <motion.span
                    ...
                    style={{
                      background: h.won
                        ? `rgba(${stateColor.winRgb},0.12)`
                        : `rgba(${stateColor.crashRgb},0.12)`,
                      color: h.won ? stateColor.win : stateColor.crash,
                      border: `1px solid ${h.won ? `rgba(${stateColor.winRgb},0.25)` : `rgba(${stateColor.crashRgb},0.25)`}`,
                    }}
                  >
                    {h.value.toFixed(2)}×
                  </motion.span>
          ...
          <div className="card-gradient-border">
            <div className="p-6">
              ...
              <motion.div
                animate={
                  state === 'crashed' ? {x: [-5, 5, -4, 4, -2, 2, 0]} : {x: 0}
                }
                ...
                <AnimatePresence mode="wait">
                  {(state === 'idle' ||
                    ...
                    <motion.button
                      ...
                      style={{
                        background: `linear-gradient(90deg, ${brand.vivid}, ${brand.soft})`,
                      }}
                    >
                  ...
                  {state === 'running' && (
                    <motion.button
                      ...
                      className="px-8 py-2.5 rounded-button font-semibold text-sm text-white cursor-pointer"
                      style={{
                        background: `linear-gradient(90deg, ${stateColor.win}, ${stateColor.winLight})`,
                      }}
                    >
