...
interface ProjectCardProps extends Pick<Project, 'url' | 'appStore' | 'playStore'> {
    name: string
    company: string
    period: string
    technologies: string[]
...
export function ProjectCard({name, url, appStore, playStore, company, period, technologies}: ProjectCardProps) {
    ...
    return (
        <>
            ...
            {modalOpen && (
                <StoreLinksModal project={project} onClose={() => setModalOpen(false)}/>
            )}
        </>
    )
}